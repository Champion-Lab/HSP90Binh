...
setwd("E:/Simon/HSP90DIA_may2024/Results/231_may23/")
...
proteins <- read.table("20240523_141927_231_may23_Report-proteins-pivot-ND-withprecursorIDcount.tsv",
                       sep = "\t",
                       header = T)
...
ggplot() +
  ...
  labs(y = "Log2 Fold Change", x = "Normalized Average Expression (Log)",
       title = "MDA-MB-231 | NDNB1182 vs DMSO") +
...
ggplot() +
  ...
  theme(panel.grid = element_blank(),
        legend.position = "none")+
  labs(y = "Log2 Fold Change", x = "Normalized Average Expression (Log)",
       title = "MDA-MB-231 | NDNB1 vs DMSO") +
