...
setwd("E:/Simon/HSP90DIA_may2024/Results/468_may23/")
...
proteins <- read.table("20240523_141927_468_may23_Report-proteins-pivot-ND-withprecursorIDcount.tsv",
                       sep = "\t",
                       header = T)
...
ggplot() +
  ...
  labs(y = "Log2 Fold Change", x = "Normalized Average Expression (Log)",
       title = "MDA-MB-468 | NDNB1182 vs DMSO") +
  ...
  geom_hline(yintercept = c(-LFCcutoff,LFCcutoff), linetype = 2) + #add lines at LFC = -1, 1
  geom_point(data = DE_1182[DE_1182$Accession %in% proteins_sig,],
...
ggplot() +
  ...
  theme(panel.grid = element_blank(),
        legend.position = "none")+
  labs(y = "Log2 Fold Change", x = "Normalized Average Expression (Log)",
       title = "MDA-MB-468 | NDNB1 vs DMSO") +
